...
排程是 **約每 12 小時一次：台北時間 09:17、21:22**（UTC 01:17、13:22），
寫在 `.github/workflows/track.yml`。兩格的分鐘不一樣，所以是兩條 cron。

**分鐘刻意不落在整點。** 整點是 GitHub Actions 的壅塞時段（官方明講 schedule 在高負載
時會延遲、每小時開頭最嚴重）。實測本 repo 用整點排程的 24 次：UTC 00:00 那格穩定延遲
**+7～13 小時**（中位數 9.4h），UTC 12:00 那格 **3.7～6.2 小時**且在惡化 —— 光是換小時
就差一倍，UTC 午夜是最壞的一格。分鐘則是更大的槓桿，`:15`/`:30`/`:45` 同樣熱門，所以
取 `:17` 和 `:22`。新排程的實際延遲還沒累積足夠樣本，別把名目時間當準。

價格追蹤早幾小時晚幾小時不影響判讀，延遲本身不是問題；真正被延遲拖出來的風險是
job 被推進更擠的時段後配不到 runner —— 2026-08-06 那次 run 就是排隊 15 分鐘後被系統
取消（`steps: []`、`runner_id: 0`，一步都沒跑）。遇到就重跑，不是程式的錯。

變動事件只記日期、不記時分，記的是 **UTC 日期**（`generated_at` 也是 UTC）。
名目時間下 UTC 日期與台北日期一致（09:17 與 21:22 都落在同一個 UTC 日），
所以同一天的兩次抓取會標同一個日期，`data/changelog.json` 每個日期最多兩批事件。
不影響比對正確性（第一次抓到的變動會寫回 `current.json`，第二次就不會重複偵測）。
晚上那次名目 UTC 13:22，延遲超過 10 小時 38 分就會跨過 UTC 午夜、標成隔天，屬正常。
...
`current.json` 裡整家層級也有一個 `needs_review`，但那是衍生的：底下任一模型或政策頁
被標，整家就會亮。儀表板**不會**為此多印一條「整家資料待覆核」—— 下面已經列出是哪一筆，
再印一條只會把單一頁面的問題講成全家有問題。只有整家自己有理由（抓取失敗、
模型數量驟減）時才印那一條。
...
排程是 12 小時一次，晚上 21:22 抓到的事情，早上 09:17 那次一跑就被清掉了。
...
- 事件型的待覆核旗標（價格暴跌、政策頁改內容）跨次留著，直到人工用 `ack.py` 確認 ——
  不會因為下一次排程跑過就自己消失。
- 每次執行具冪等性，可重複安全執行。
- 政策頁雜湊算的是**主文的可見文字**（`<article>` / `<main>` 那一塊），不是整份 HTML，
  也不含導覽列與頁尾。build id 與 nonce 藏在 script 裡每天都變；頁尾的語言選單、
  cookie 提示、地區字串則會隨抓取來源所在地而變 —— 排程跑在 GitHub 的美國 runner，
  手動跑在自己的機器上，雜湊整頁會讓兩邊互相誤報成「政策變動」。
- 雜湊口徑記在每個政策頁的 `hash_method` 欄位。日後再改抽取方式時，
  一併改 `base.POLICY_HASH_METHOD`：口徑不同的雜湊沒有可比性，
  diff 會重新取基準，不會因為換算法就四家一起假警報。

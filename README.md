...
排程是 **每 12 小時一次：台北時間 07:50、19:50**（UTC 23:50、11:50），
寫在 `.github/workflows/track.yml`。

時間刻意挑非整點、且避開 UTC 午夜。`0 0 * * *` 是 GitHub Actions 最壅塞的排程格，
官方明講 schedule 在高負載時會延遲、每小時開頭最嚴重 —— 這個 repo 用 `0 0 * * *`
的那兩週，實際開跑時間穩定落在 **+7～13 小時**（名目 UTC 00:00，實際都在 09:00-12:40 UTC），
沒有一次接近準時。挪到冷門時段後延遲通常只剩幾分鐘。

變動事件只記日期、不記時分，而且記的是 **UTC 日期**（`generated_at` 也是 UTC）。
UTC 日期在台北時間 08:00 才跳，所以早上 07:50 那次會標成前一天 ——
台北 8/2 早上抓到的變動，changelog 上寫的是 `2026-08-01`。

換句話說，同一個 UTC 日期底下的兩次抓取是「台北某天傍晚 19:50」＋「隔天早上 07:50」，
兩次都標同一個日期，`data/changelog.json` 每個日期最多因此出現兩批事件。
...
排程是 12 小時一次，晚上 19:50 抓到的事情，早上 07:50 那次一跑就被清掉了。
